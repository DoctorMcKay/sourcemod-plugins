...
public Action:TF2Items_OnGiveNamedItem(client, String:classname[], defindex, &Handle:item) {
	if(defindex == WEAPON_JUMPER) {
		...
		if(diff <= 0.0) {
			TF2Items_SetAttribute(item, 0, ATTRIBUTE_MINUS_BOMBS, diff);
			TF2Items_SetNumAttributes(item, 1);
		} else {
			TF2Items_SetAttribute(item, 0, ATTRIBUTE_ADD_BOMBS, diff);
			TF2Items_SetAttribute(item, 1, ATTRIBUTE_MINUS_BOMBS, 0.0);
